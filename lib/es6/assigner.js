...
class IssueAssigner {
  ...
  getNextTriager (ourRepo:Object):string {
    ...
    log(`Valid triagers for ${repoStr} are: ${ourRepo.triagers.join(", ")}`,
        ourRepo.repo);
  ...
  getRepoForAssignment(data:Object):Object {
    ...
    log(`[#${data.issue.number}] Examining issue (${data.issue.url})`,
        ourRepo.repo);
    if (ourRepo === null) {
      log("Got an issue for a repo we don't care about, doing nothing",
          ourRepo.repo, "debug");
      return null;
    }
    if (data.issue.pull_request) {
      log("Issue is a pull request, ignoring", ourRepo.repo, "debug");
      return null;
    }
    if (data.action !== 'opened') {
      log(`[#${data.issue.number}] Issue is not new, ignoring`,
          ourRepo.repo, "debug");
      return null;
    }
    if (data.issue.assigne === null) {
      log(`[#${data.issue.number}] Issue already has someone assigned, ` +
          `ignoring`, ourRepo.repo, "debug");
      return null;
    }
    return ourRepo;
  }
  ...
  async performAssignment(ourRepo:Object, issueNum:number, reporter:string):void {
    ...
    log(`[#${issueNum}] Issue was reported by ${reporter}`, ourRepo.repo);
    if (ourRepo.autoAssign && ourRepo.triagers.length) {
      ...
      log(`[#${issueNum}] Assigning issue to ${randTriager}`, ourRepo.repo);
    ...
    if (randTriager) {
      log(`[#${issueNum}] Successfully assigned ${randTriager} as the triager`, 
          ourRepo.repo);
    }
  }
  ...
  async assignOnGithub (ourRepo:Object, issueNum:number, triager:string,
      labels:Array):void {
    let repoStr:string = this.repoStr(ourRepo);
    log(`[#${issueNum}] Updating issue assignment on github`, ourRepo.repo);
    ...
    if (labels) {
      ...
      log(`[#${issueNum}] Setting labels: ${updateObj.labels.join(', ')}`, 
          ourRepo.repo);
    }
    ...
    if (triager !== "" && updated.assignee.login !== triager) {
      log(`[#${issueNum}] Github replied OK but triager wasn't set yet`,
          ourRepo.repo, "warn");
    }
  }
  ...
  async writeAssignmentCache (ourRepo:Object):void {
    ...
    log(`Writing assignment cache to ${cacheFile}`, ourRepo.repo, "info");
...
class OldIssueAssigner extends IssueAssigner {
  ...
  async issuesWithoutMilestones (repo:Object):Array {
    let repoStr = this.repoStr(repo);
    log(`Getting all open, unassigned, unmilestoned issues from github ` +
        `for the repo: ${repoStr}`, repo.repo);
    let client = octonode.client(process.env.TRIAGER_TOKEN);
    ...
    let allIssues = [];
    while (!doneWithPages) {
      log(`Getting page: ${curPage}`, repo, "debug");
  ...
  async assignOldIssues ():void {
    log("Assigning old issues for all configured repos");
    for (let repo of this.repos) {
      log(`Assigning old issues for repo: ${this.repoStr(repo)}`);
      ...
      if (issues.length === 0) {
        log("There were no issues to assign");
        continue;
      }
      log(`Assigning ${issues.length} old issues`);
      for (let issue of issues) {
        log(`Assigning old issue #${issue.number}`);
